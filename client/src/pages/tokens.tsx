      ...
      <div className="space-y-6">
        ...
        {trendingTokens.length > 0 && (
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <h2 className={`text-lg font-black flex items-center gap-2 ${privateMode ? "text-[#10B981]" : "text-gray-900"}`}>
                <Flame className={`w-5 h-5 ${privateMode ? "text-[#10B981]" : "text-red-500"}`} />
                {privateMode ? "> NOW_TRENDING" : "Now Trending"}
              </h2>
              <div className="flex gap-1">
                <button 
                  onClick={() => scrollTrending("left")} 
                  className={`p-1.5 border-2 rounded transition-all ${privateMode ? "border-[#10B981]/30 text-[#10B981] hover:border-[#10B981]" : "border-gray-300 hover:border-black"}`}
                ...
                <Link key={token.mint} href={`/token/${token.mint}`}>
                  <motion.div 
                    ...
                    className={`flex-shrink-0 w-72 p-3 cursor-pointer transition-all ${
                      privateMode 
                        ? "bg-black border-2 border-[#10B981]/50 hover:border-[#10B981]" 
                  ...
                  >
                    <div className="flex gap-3">
                      <div className={`w-16 h-16 rounded-lg overflow-hidden flex-shrink-0 border ${privateMode ? "border-[#10B981]/30" : "border-black/20"}`}>
                        {token.imageUri ? (
                          <img src={token.imageUri} alt={token.name} className="w-full h-full object-cover" />
                        ) : (
                          <div className={`w-full h-full flex items-center justify-center font-black text-xl ${privateMode ? "bg-black text-[#10B981]" : "bg-red-700 text-white"}`}>
                            {token.symbol[0]}
                          </div>
                        )}
                      </div>
                      <div className="flex-1 min-w-0">
                        <div className={`font-bold text-sm truncate ${privateMode ? "text-[#10B981]" : "text-white"}`}>
                          {token.name}
                        </div>
                        <div className={`text-xs ${privateMode ? "text-[#10B981]/70" : "text-white/80"}`}>
                          ${token.symbol}
                        </div>
                        ...
                        <div className={`text-xs ${privateMode ? "text-[#10B981]/50" : "text-white/70"}`}>
          ...
          <Link href="/create">
            <motion.button
              ...
              className={`flex items-center gap-2 font-bold px-5 py-2.5 border-2 transition-all ${
                privateMode 
                  ? "bg-[#10B981] text-black border-[#10B981] hover:shadow-[0_0_15px_rgba(57,255,20,0.3)] font-mono" 
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              ...
              <button
                ...
                className={`flex items-center gap-1.5 px-3 py-1.5 text-sm font-bold border-2 transition-all ${
                  activeFilter === tab.id
                    ...
                    : privateMode 
                      ? "bg-black text-[#10B981]/70 border-[#10B981]/30 hover:border-[#10B981]" 
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="relative flex-1 sm:w-48">
              <Search className={`absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 ${privateMode ? "text-[#10B981]/50" : "text-gray-400"}`} />
              <input
                type="text"
                value={searchQuery}
            ...
            {/* View Toggle */}
            <div className={`flex border-2 ${privateMode ? "border-[#10B981]/30" : "border-gray-300 rounded-lg overflow-hidden"}`}>
              <button
                onClick={() => setViewMode("grid")}
                className={`p-2 transition-all ${
                  viewMode === "grid"
                    ? privateMode ? "bg-[#10B981] text-black" : "bg-black text-white"
                    : privateMode ? "bg-black text-[#10B981]/50" : "bg-white text-gray-400"
                }`}
        ...
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 text-center">
            <AlertCircle className={`w-12 h-12 mb-4 ${privateMode ? "text-[#10B981]" : "text-red-500"}`} />
            <p className={privateMode ? "text-[#10B981]/70" : "text-gray-500"}>Failed to load tokens</p>
          </div>
        ) : sortedTokens.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 text-center">
            <Rocket className={`w-16 h-16 mb-4 ${privateMode ? "text-[#10B981]/30" : "text-gray-300"}`} />
            <h3 className={`text-xl font-bold mb-2 ${privateMode ? "text-white" : "text-gray-700"}`}>
              {searchQuery ? "No tokens found" : "No tokens yet"}
            </h3>
            <p className={`mb-6 max-w-md ${privateMode ? "text-[#10B981]/50" : "text-gray-500"}`}>
                    ...
                    <Link key={token.mint} href={`/token/${token.mint}`}>
                      <tr 
                        className={`cursor-pointer border-b transition-all ${
                          privateMode 
                            ? "border-[#10B981]/20 hover:bg-[#10B981]/10" 
                            : "border-gray-100 hover:bg-gray-50"
                        }`}
                        data-testid={`table-row-${token.mint}`}
                      >
                        <td className={`py-3 px-4 font-mono text-sm ${privateMode ? "text-[#10B981]/50" : "text-gray-400"}`}>
                          #{index + 1}
                        </td>
                        <td className="py-3 px-4">
                          <div className="flex items-center gap-3">
                            <div className={`w-10 h-10 rounded-lg overflow-hidden flex-shrink-0 border ${privateMode ? "border-[#10B981]/30" : "border-gray-200"}`}>
                              {token.imageUri ? (
                                <img src={token.imageUri} alt={token.name} className="w-full h-full object-cover" />
                              ) : (
                                <div className={`w-full h-full flex items-center justify-center font-bold ${privateMode ? "bg-black text-[#10B981]" : "bg-gray-100 text-red-500"}`}>
                                  {token.symbol[0]}
                                </div>
                              )}
                            </div>
                            <div>
                              ...
                              <div className={`text-xs ${privateMode ? "text-[#10B981]/50" : "text-gray-500"}`}>
                                ${token.symbol}
                              </div>
                            </div>
                          </div>
                        ...
                        <td className="py-3 px-4 text-right">
                          <div className="flex items-center justify-end gap-2">
                            <div className={`w-16 h-2 rounded-full overflow-hidden ${privateMode ? "bg-black border border-[#10B981]/30" : "bg-gray-200"}`}>
                              <div 
                                className={`h-full ${
                                  privateMode 
                                    ? "bg-[#10B981]" 
                            ...
                            <span className={`text-xs font-mono ${privateMode ? "text-[#10B981]" : "text-gray-600"}`}>
                              {token.bondingCurveProgress.toFixed(0)}%
                            </span>
                          </div>
                        </td>
                        <td className={`py-3 px-4 text-right text-sm ${privateMode ? "text-[#10B981]/70" : "text-gray-500"}`}>
                          {getTimeAgo(new Date(token.createdAt))}
                        </td>
                        <td className={`py-3 px-4 text-right text-sm font-mono ${privateMode ? "text-[#10B981]/50" : "text-gray-400"}`}>
