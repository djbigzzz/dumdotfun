...
export function PrivacyWallet() {
  ...
  return (
    <div className={`border-2 border-black rounded-xl p-6 space-y-5 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] ${
      privateMode ? "bg-black border-[#4ADE80]" : "bg-gradient-to-br from-purple-100 to-indigo-100"
    }`}>
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <Shield className={`w-5 h-5 ${privateMode ? "text-[#4ADE80]" : "text-purple-600"}`} />
          <h2 className={`text-lg font-black uppercase ${privateMode ? "text-[#4ADE80] font-mono" : "text-gray-900"}`}>
        ...
        <div className="flex items-center gap-2">
          <motion.button
            ...
            className={`p-2 rounded-lg transition-colors ${
              privateMode ? "text-[#4ADE80] hover:bg-[#4ADE80]/10" : "text-gray-600 hover:bg-gray-200"
            }`}
      ...
      <div className={`grid grid-cols-2 gap-3 p-4 rounded-lg border-2 ${
        privateMode ? "bg-zinc-900/50 border-[#4ADE80]/30" : "bg-white/50 border-gray-200"
      }`}>
        <div>
      ...
      <div className={`border-t-2 pt-4 ${privateMode ? "border-[#4ADE80]/30" : "border-gray-200"}`}>
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            <Key className={`w-4 h-4 ${privateMode ? "text-[#4ADE80]" : "text-purple-600"}`} />
            <span className={`text-sm font-bold uppercase ${privateMode ? "text-[#4ADE80]/60 font-mono" : "text-gray-500"}`}>
        ...
        <p className={`text-xs mb-3 ${privateMode ? "text-[#4ADE80]/40 font-mono" : "text-gray-500"}`}>
          {privateMode 
            ...
            {stealthAddresses.map((stealth, i) => (
              ...
              >
                <div className="flex-1 min-w-0">
                  ...
                  <p className={`text-[10px] ${privateMode ? "text-[#4ADE80]/40" : "text-gray-400"}`}>
          ...
          >
            <motion.div
              ...
              className={`w-full max-w-md p-6 rounded-xl border-2 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] ${
                privateMode ? "bg-zinc-900 border-[#4ADE80]" : "bg-white"
              }`}
            >
              <h3 className={`text-xl font-black mb-4 ${privateMode ? "text-[#4ADE80] font-mono" : "text-gray-900"}`}>
                {privateMode ? "PRIVATE_DEPOSIT" : "Private Deposit"}
              </h3>
              <p className={`text-sm mb-4 ${privateMode ? "text-[#4ADE80]/60 font-mono" : "text-gray-600"}`}>
                {privateMode 
              ...
              <input
                ...
                className={`w-full px-4 py-3 rounded-lg border-2 border-black mb-4 font-mono ${
                  privateMode ? "bg-black text-white border-[#4ADE80]/50" : "bg-gray-100"
                }`}
